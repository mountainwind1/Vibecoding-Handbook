...
## [v7.1] - 2026-09-25
> 在 v7 手册内以增补形式发布，**非破坏性**。两块，都来自真实项目：① **流程瘦身**——依据三个真实项目 origin/main 的只读实测，收紧分档、收口按风险分级、证据设上限、DECISIONS 每次收口归档；已同步到这三个项目（各自 PR 合并、CI 绿），**还没有项目按新规则跑完一次收口**，效果待下一轮收口实测。② **`map` 项目地图**（试验，不进默认路径）——已装进三个真实项目，发布在自托管服务器上。
### 升级（从 v7）
- 覆盖 skill：`skills/{kof,close,xreview}` 与 `agents/security-auditor.md` 原样复制。项目自己改过 kof 的，手工插入四处：Evidence ≤ 800 字、DECISIONS 只读索引与最新 2–3 条、收口 A5 触发条件与复测上限、收口时归档 DECISIONS。
- 项目规则按模板对齐：`CLAUDE.md` TIER 1 第 9 条（信任边界）、「命门清单」的判据与「信任边界」一行（命门清单是领域知识，按新判据复核，不要机械删减）；`SELFCHECK.md` R1.9 / R3.*；`ENGINEERING.md` 证据链；`DECISIONS.md` 头部。PLAN.md 与 DECISIONS 历史条目不用动——首次归档在下次收口（`close` 第 5 步）自然发生。
- （可选）`map`：复制 `skills/map/`，在 `ENGINEERING.md` 加「模块地图」一节；要云端再加「发布：」行（见其 SKILL.md）。
### 变更（流程瘦身：依据三个真实项目 origin/main 的只读实测）
> 起因：用户的原话是"很多功能设计得太小心、太精细，浪费了大量 token 和时间"。四项按省下的量排序，每项注明属于哪类问题（没写 / 写了不执行 / 太重被绕过）。
- **分档收紧**（太重）：归档口诀从"出错代价大 / 逻辑复杂 / 不可逆，任一为是即重型"改为"**出错后不能靠回滚代码恢复**才是重型"（动数据、动钱、动安全边界、动对外契约），并写明动的是什么（`（命门：…）`）；"代价大""逻辑复杂"靠测试兜底、不升档；一个里程碑通常 1–2 个命门。实测：两个项目重型占 58% / 76%、标"单独 / 命门"占 55% / 83%——第一问太主观，大部分任务都走了全套重流程；重型占 24% 的项目文档与流程开销最低（占文字改动 16%，另两个 21% / 59%）。手册正文、`templates/PLAN.md`、`templates/CLAUDE.md` 同步。
- **收口按风险分级**（太重）：A5 的触发从"碰到鉴权 / 支付 / PII / 上传 / 用户输入入库任一项"改为"**新增或改动了信任边界**"（新的鉴权 / 权限路径、新的外部输入口、动钱、新增或扩散 PII），只在既有边界内改业务逻辑的不跑、收口报告写明依据；复测**最多一轮**，复测又见高危就停下报告用户，不进"修 → 测 → 修"循环；**做了 A5 就不再跑 `xreview`**；旅程走查只重走本里程碑改动的那段。实测：一个项目某里程碑收口 8 次提交、6.6 小时（做任务 9.3 小时），近三个里程碑收口各产出 1,040–1,466 行文档。`close`、`xreview`、`security-auditor`、手册 3.5 / 5.1 与模板同步。
- **证据设上限**（没写）：每任务 Evidence ≤ 800 字（做了什么 / 关键取舍 / 实测数字 / 待拍板·偏差），逐条输出放 PR 描述；证据产物进仓库只留报告与结果摘要，原始记录与大文件外置、仓库留 SHA-256 清单，能机制化就上"新增文件大小"门禁——这是一个下游项目自己踩坑后定下的证据政策，上移进手册。实测：该项目任务证据包占全部文字改动 46%、另有 143 万行机器产物进了仓库；另一个项目每任务 Evidence 从中位 1,510 字涨到最近平均 3,038 字。手册证据链、`kof` 收尾、模板 PLAN / ENGINEERING 同步。
- **DECISIONS 每次收口归档**（写了不执行）：手册早就写了"DECISIONS 分卷"，但时机是"满一个 Phase"（多数项目不分 Phase）、又没写进收口的固定动作，三个项目的 DECISIONS 仍涨到 7.7 万–21 万字。改为 `close` 第 5 步的固定动作：每次收口把已失效、被取代、已兑现的条目移入 `DECISIONS-ARCHIVE.md`（只移不删、索引全量保留），`kof` 只读索引与最新 2–3 条、不读归档。
### 新增（试验）
- **`map` skill · 项目地图**（试验中，不进默认路径）：进度块回答"做到第几个任务"，回答不了"在业务流程的哪一步、问题出在哪块、精力花在哪"。脚本从 PLAN.md + `ENGINEERING.md`「模块地图」（新增的可选一节：业务线 + 「模块 × 层 × 路径」表）+ git 现算，出一个自带页面（阶段 / 线路图 / 模块卡片 / 问题原文 / 投入热力图）；装了 [archify](https://github.com/tt-a1i/archify) 时同一份事实另画成交互架构图（布局由脚本按网格算好，不让 agent 手摆坐标；「你在这里 / 问题在哪 / 投入最多 / 跨仓库依赖」四个章节；节点可跳源码）。PLAN 读法与 `prog` 同口径，自检里交叉核对。`kof` 任务收尾静默刷新；`--publish` 执行项目自配的发布命令（托管不绑定任何一家 agent 工具），**发布后像匿名浏览器一样打开网址，能看到地图就失败**（页面有安全欠账原文）；托管：有自己的服务器就用它——**已实测**（阿里云 ECS + Docker 里的 Caddy）：只能收 tar 包的发布账号（SSH 强制命令 `server/vibe-map-receive`，跑命令 / 跳目录 / 开 shell / 转发端口均被拒）+ 用户自己设的口令（`server/vibe-map-set-password`，agent 不经手）+ HTTPS，发布一条命令约 8 秒；没有就 Cloudflare Workers 静态资源 + Cloudflare Access（按官方文档核实，未实测；GitHub Pages 从私有仓库发布照样公开，排除）。页面不从第三方加载任何资源（原型曾从 Google Fonts 拉字体：私有页面对外发请求，大陆访问还会卡住渲染），自检锁住。
- **首次试用反馈后改版**：①站点根是**多项目总入口**（每个项目一行：正在做什么、第几步、要不要你到场、这一步最要紧的问题，卡在用户这里的排前）；②项目页**默认先看「进展」**——当前功能的步骤条、当前这一步、这一步的问题（待拍板 / 这一步的偏差 / CI 失败在前，别的任务上的偏差折叠）、最近几次提交；架构图退到「模块图」页签。原因：图挺花，但"正在开发的功能在哪一步、这一步的问题是什么"没展示。
