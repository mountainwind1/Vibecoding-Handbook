...
| [skills/map](skills/map/SKILL.md) | **试验中。** 项目地图：多项目总入口 + 每个项目「正在开发的功能在哪一步、这一步的问题」，另有业务流程上的模块、每个模块的页面 / 接口 / 数据情况、问题挂在哪、精力花在哪（模块 × 里程碑投入热力图）——脚本从 PLAN + `ENGINEERING.md`「模块地图」+ git 现算；装了 [archify](https://github.com/tt-a1i/archify) 另出交互架构图；`--publish` 执行项目自配的发布命令 |
